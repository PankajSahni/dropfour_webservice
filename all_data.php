...
if(isset($json_data) && count($json_data) && $json_data['fb_id']!=''){
	...
	$game_invite_sql = "select g.id game_id, * from game g inner join user u on g.player_one=u.id where g.player_two in (select id from user u where u.fb_id = '".$fb_id."' and g.status=0)";
